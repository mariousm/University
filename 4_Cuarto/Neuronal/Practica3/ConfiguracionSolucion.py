...
def configuraPoblacion(toolbox):
	...
	tamanos_videos = sum(dg.memorias)
	probabilidad = (((tamanos_videos/dg.videos)/dg.tam_cache)*dg.caches)/(dg.videos*dg.caches)
	toolbox.register("attribute", generarGenotipoValido, probabilidad) #En realidad, se indica que serán entereos entre 0 y 1
	''' El individuo se crea como una lista (o repeticion) de "attribute", definido justo antes
	Tendrá una longitud de 5 atributos '''
	toolbox.register("individual", tools.initRepeat, creator.Individual, toolbox.attribute, n=dg.caches*dg.videos)
...
def generarGenotipoValido(probabilidad):
